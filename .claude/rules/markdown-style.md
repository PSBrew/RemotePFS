...
- Iconography: use UTF-8 emoji for section headings and quick-links. Keep icons consistent and semantically appropriate. Examples:
  ...
  - 🔗 Related projects
  - 💖 Sponsor
- Badges: prefer shields.io for badges. Style: `style=for-the-badge` for the top cluster. Include: status, license, Python version. Use flat-square for a second-row group when needed. RemotePFS is not on PyPI and has no published docs site yet; do not add PyPI or docs badges.
...
- Center the hero block. Include in order:
  ...
  3. One-line product sentence (H3 or short paragraph) that expands the acronym and lists delivery surfaces when relevant (service, HTTP API, USB gadget).
  4. Top badge cluster (status, PyPI, license, python, docs).
  5. Quick-links row (emoji-prefixed links separated by middot ·). Order:
     - 📚 Documentation · 📦 Install · ⌨️ Commands · 💙 Thanks · 🔗 Related projects
     - then a blank line
